...
#    print(clean_line)
    ...
    for count in range(len(line)):
        for divider in range(len(line)):
            # Assuming we can't try to divide the number by itself
            if count == divider:
                continue

            if int(line[count]) % int(line[divider]) == 0:
                total += int(line[count]) / int(line[divider])
                break
