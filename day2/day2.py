...
for ss_line in spreadsheet.splitlines():
    # colapse multiple spaces into a single space
    clean_line = re.sub(r" +", " ", ss_line)
#    print(clean_line)

    biggest_number = 0
    smallest_number = 0
    for number in clean_line.split(" "):
        if (int(number) > int(biggest_number)) or (biggest_number == 0):
            biggest_number = number
        if (int(number) < int(smallest_number)) or (smallest_number == 0):
            smallest_number = number
#        print("current biggest: {}  current smallest: {}".format(biggest_number, smallest_number))

#    print("biggest_number: {}  smallest_number: {}".format(biggest_number, smallest_number))

    total += int(biggest_number) - int(smallest_number)
